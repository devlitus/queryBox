...
export default function RequestBar() {
  ...
  return (
    <div class="p-4 flex flex-col md:flex-row md:items-center gap-2 md:gap-0">
      {/* Method selector + URL input + variable indicator */}
      <div class="flex items-center gap-0 flex-1">
        <MethodSelector />
        <div class="flex-1 flex items-center bg-pm-bg-tertiary border-y border-pm-border md:border-r-0 rounded-r md:rounded-r-none focus-within:border-pm-accent">
          <input
            ref={urlInputRef}
            type="url"
            placeholder="Enter a URL (e.g., https://jsonplaceholder.typicode.com/todos/1)"
            value={url}
            class="flex-1 px-4 py-2 bg-transparent text-sm text-pm-text-primary placeholder:text-pm-text-tertiary focus:outline-none"
            aria-label="Request URL"
            onInput={(e) => updateUrl((e.target as HTMLInputElement).value)}
          />
          {showVariableIndicator && (
            <div class="px-2">
              <VariableIndicator text={url} variables={activeVariablesMap.value} />
            </div>
          )}
        </div>
      </div>
